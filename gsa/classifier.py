...
class ClassifierBySequencePatterns:
    ...
    def important_rules(self, label=0, dict_of_coding={'work': 1,
        ...
        if self.model:
            ...
            dict_of_uncoding = {}
            for key, value in dict_of_coding.items():
                dict_of_uncoding[value] = key
    ...
    def _classify_object(self, object_to_classification, silence=True):
        score_for_class = [0 for _ in range(self.number_of_classes)]

        rules_from_class = [[] for _ in range(self.number_of_classes)]
        ...
        for i in range(self.number_of_classes):
            ...
            id_to_remove_from_class = [[] for i in range(self.number_of_classes)]
    ...
    #
    #         try:
    #             for rule_id in self.rules_class[class_name].dict_of_contributions_to_score_class.keys():
...
class ClassifierByClosureSequencePatternsWithWeighs(ClassifierByClosureSequencePatterns):
    ...
    def _classify_object(self, object_to_classification, silence=True):
        ...
        for i in range(len(score_for_class)):
...
#     def _classify_object(self, object_to_classification, silence=True):
#         score_for_each_class = [0 for _ in range(self.number_of_classes)]
#         score_for_class = [0 for _ in range(self.number_of_classes)]
#
#         tree = self.trie
#
